...
class ImageModelVersion:
    ...
    def Generator(self):
        ...
        down_stack = [
            ...
            self.downsample(512, 4, apply_dropout=True, dropout_value=0.5),  # (batch_size, 4, 4, 512)
            self.downsample(512, 4, apply_dropout=True, dropout_value=0.5),  # (batch_size, 2, 2, 512)
            self.downsample(512, 4, apply_dropout=True, dropout_value=0.5),  # (batch_size, 1, 1, 512)  
        ]

        up_stack = [
            self.upsample(512, 4),  # (batch_size, 2, 2, 1024)
            self.upsample(512, 4),  # (batch_size, 4, 4, 1024)
            self.upsample(512, 4),  # (batch_size, 8, 8, 1024)
    ...
    def downsample(self, filters, size, apply_batchnorm=True, apply_dropout=False, dropout_value=0.5, strides_num=2):
        #add apply drop out
        initializer = tf.random_normal_initializer(0., 0.02)
