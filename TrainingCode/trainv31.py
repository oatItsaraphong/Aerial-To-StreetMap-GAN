...
def Generator():
  ...
  down_stack = [
    ...
    downsample(512, 4, apply_dropout=True, dropout_value=0.5),  # (batch_size, 4, 4, 512)
    downsample(512, 4, apply_dropout=True, dropout_value=0.5),  # (batch_size, 2, 2, 512)
    downsample(512, 4, apply_dropout=True, dropout_value=0.5),  # (batch_size, 1, 1, 512)
  ]
...
# checkpoint
checkpoint_dir = './training_checkpointsv31'
...
summary_writer = tf.summary.create_file_writer(
  log_dir + "fitv31/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S"))
